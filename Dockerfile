...
# estándar (ej. 8082 en desarrollo local) redirigiría al mismo número de
...
RUN echo '<Directory /var/www/html>\n\
    ...
    AllowOverride All\n\
    Require all granted\n\
</Directory>' > /etc/apache2/conf-available/sporttime.conf \
    && a2enconf sporttime
...
RUN printf '%s\n' \
    ...
    '<VirtualHost *:443>' \
    '    ServerName localhost' \
    '    DocumentRoot /var/www/sporttime/public' \
    '    SSLEngine on' \
    '    SSLCertificateFile /etc/apache2/ssl/certificate.crt' \
    '    SSLCertificateKeyFile /etc/apache2/ssl/private.key' \
    '    <Directory /var/www/sporttime/public>' \
    '        Options -Indexes +FollowSymLinks' \
    '        AllowOverride All' \
    '        Require all granted' \
    '    </Directory>' \
    '</VirtualHost>' \
    '</IfModule>' > /etc/apache2/sites-available/sporttime-ssl.conf \
    && a2ensite sporttime-ssl
...
COPY . /var/www/sporttime/
WORKDIR /var/www/sporttime
...
RUN sed -i 's|/var/www/html|/var/www/sporttime/public|g' /etc/apache2/conf-available/sporttime.conf

# Permisos
RUN chown -R www-data:www-data /var/www/sporttime \
    && find /var/www/sporttime -type d -exec chmod 755 {} \; \
    && find /var/www/sporttime -type f -exec chmod 644 {} \;
