#!/usr/bin/env bash
# ============================================================
# install.sh — Instalación de SportTime en AlmaLinux 9.8
# (httpd + PHP + MariaDB, con SELinux y firewalld).
#
# Despliegue "Opción A": el DocumentRoot apunta a public/, dejando
# el resto del código (.env, app/, core/, ...) FUERA del alcance web.
#
# Toda la configuración se toma del .env del proyecto (no se duplica).
# Idempotente: se puede re-ejecutar sin romper una instalación previa.
#
# Uso (como root):   sudo bash scripts/install.sh
# Opciones:
#   --no-deps        No instalar paquetes del sistema (dnf)
#   --no-vhost       No configurar el VirtualHost de Apache (httpd)
#   --no-firewall    No tocar firewalld
#   --no-selinux     No aplicar contextos/booleans de SELinux
#   --root-pass PWD  Contraseña de root de MySQL/MariaDB (si no usa auth por socket)
#
# Nota: para entornos con Docker, usá  `docker compose up -d`  en su lugar.
# ============================================================
set -euo pipefail

# ---- Rutas ----
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
ENV_FILE="$PROJECT_ROOT/.env"

# [ALMA] En RHEL/AlmaLinux Apache corre como usuario/grupo "apache", no "www-data"
WEB_USER="apache"
WEB_GROUP="apache"

# ---- Flags ----
INSTALL_DEPS=1; CONFIG_VHOST=1; CONFIG_FIREWALL=1; CONFIG_SELINUX=1; MYSQL_ROOT_PASS=""
while [ $# -gt 0 ]; do
  case "$1" in
    --no-deps)     INSTALL_DEPS=0 ;;
    --no-vhost)    CONFIG_VHOST=0 ;;
    --no-firewall) CONFIG_FIREWALL=0 ;;
    --no-selinux)  CONFIG_SELINUX=0 ;;
    --root-pass)   MYSQL_ROOT_PASS="${2:-}"; shift ;;
    *) echo "Opción desconocida: $1" >&2; exit 1 ;;
  esac
  shift
done

# ---- Requiere root ----
if [ "$(id -u)" -ne 0 ]; then
  echo "ERROR: ejecutá como root  ->  sudo bash scripts/install.sh" >&2
  exit 1
fi

# ---- Cargar .env (ignora comentarios/blancos; soporta CRLF) ----
[ -f "$ENV_FILE" ] || { echo "ERROR: no existe $ENV_FILE" >&2; exit 1; }
while IFS= read -r line || [ -n "$line" ]; do
  line="${line%$'\r'}"
  case "$line" in ''|\#*) continue ;; esac
  [ "${line#*=}" != "$line" ] || continue
  key="${line%%=*}"; val="${line#*=}"
  val="${val%\"}"; val="${val#\"}"; val="${val%\'}"; val="${val#\'}"
  export "$key=$val"
done < "$ENV_FILE"

: "${APP_NAME:?falta APP_NAME en .env}"
: "${DB_NAME:?falta DB_NAME en .env}"
: "${DB_USER:?falta DB_USER en .env}"
: "${DB_PASS:?falta DB_PASS en .env}"
DB_HOST="${DB_HOST:-localhost}"
APP_URL="${APP_URL:-http://localhost}"
PROJECT="$DB_NAME"   # identificador para el VirtualHost / conf

# ---- Parsear APP_URL -> ServerName + puerto ----
url_noproto="${APP_URL#*://}"
SERVER_NAME="${url_noproto%%[:/]*}"
case "$url_noproto" in
  *:*) HTTP_PORT="${url_noproto#*:}"; HTTP_PORT="${HTTP_PORT%%/*}" ;;
  *)   HTTP_PORT=80 ;;
esac
[ -n "$SERVER_NAME" ] || SERVER_NAME="localhost"

echo "==> Proyecto:   $APP_NAME ($PROJECT)"
echo "==> Código:     $PROJECT_ROOT   (DocumentRoot: $PROJECT_ROOT/public)"
echo "==> Base:       $DB_NAME · usuario $DB_USER · host $DB_HOST"
echo "==> Apache:     $SERVER_NAME : $HTTP_PORT"
echo

# ---- 1) Dependencias del sistema ----
if [ "$INSTALL_DEPS" -eq 1 ]; then
  echo "==> Instalando paquetes (httpd, php, mariadb)..."
  # [ALMA] dnf en vez de apt-get. AlmaLinux 9 trae PHP 8.1 en AppStream;
  # si necesitás otra versión, habilitá el módulo antes:
  #   dnf module list php
  #   dnf module enable php:8.3
  dnf install -y epel-release
  dnf install -y httpd mariadb-server mariadb \
      php php-cli php-mysqlnd php-gd php-mbstring php-xml php-common policycoreutils-python-utils

  systemctl enable --now httpd
  systemctl enable --now mariadb
else
  echo "==> (--no-deps) Se omite la instalación de paquetes."
fi

# ---- 2) Base de datos + usuario (privilegios mínimos) ----
if [ "$DB_HOST" = "localhost" ] || [ "$DB_HOST" = "127.0.0.1" ]; then
  echo "==> Configurando base de datos local..."
  # [ALMA] mariadb-server de AlmaLinux NO usa auth por socket unix_socket para
  # root por defecto (a diferencia de MariaDB de Debian/Ubuntu). En una
  # instalación nueva root no tiene contraseña salvo que hayas corrido
  # mysql_secure_installation. Probamos primero sin password y si falla,
  # usamos --root-pass.
  if [ -n "$MYSQL_ROOT_PASS" ]; then
    MYSQL=(mysql -u root -p"$MYSQL_ROOT_PASS")
  else
    MYSQL=(mysql -u root)
  fi

  if ! "${MYSQL[@]}" -e "SELECT 1;" >/dev/null 2>&1; then
    echo "ERROR: no se pudo conectar a MariaDB como root." >&2
    echo "       Corré 'sudo mysql_secure_installation' o pasá --root-pass PWD." >&2
    exit 1
  fi

  "${MYSQL[@]}" <<SQL
CREATE DATABASE IF NOT EXISTS \`$DB_NAME\` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
CREATE USER IF NOT EXISTS '$DB_USER'@'localhost' IDENTIFIED BY '$DB_PASS';
CREATE USER IF NOT EXISTS '$DB_USER'@'%'         IDENTIFIED BY '$DB_PASS';
GRANT SELECT, INSERT, UPDATE, DELETE ON \`$DB_NAME\`.* TO '$DB_USER'@'localhost';
GRANT SELECT, INSERT, UPDATE, DELETE ON \`$DB_NAME\`.* TO '$DB_USER'@'%';
FLUSH PRIVILEGES;
SQL

  # Cargar schema + seed SOLO si la base está vacía (idempotente)
  TABLES=$("${MYSQL[@]}" -N -e "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema='$DB_NAME';")
  if [ "${TABLES:-0}" -eq 0 ]; then
    echo "==> Base vacía: cargando schema.sql (como root)..."
    "${MYSQL[@]}" "$DB_NAME" < "$PROJECT_ROOT/database/schema.sql"
    if [ -f "$PROJECT_ROOT/database/seed_sporttime.sql" ]; then SEED="seed_sporttime.sql"; else SEED="seed.sql"; fi
    if [ -f "$PROJECT_ROOT/database/$SEED" ]; then
      echo "==> Cargando seed: $SEED"
      "${MYSQL[@]}" "$DB_NAME" < "$PROJECT_ROOT/database/$SEED"
    fi
  else
    echo "==> '$DB_NAME' ya tiene $TABLES tablas: no se recarga schema/seed."
  fi
else
  echo "==> DB_HOST=$DB_HOST es remoto: se omite la creación local."
  echo "    Creá la base '$DB_NAME' y el usuario '$DB_USER' en el servidor de BD"
  echo "    (podés usar database/create_db_user.sql como referencia)."
fi

# ---- 3) Permisos ----
echo "==> Ajustando permisos ($WEB_USER)..."
chown -R "$WEB_USER:$WEB_GROUP" "$PROJECT_ROOT"
find "$PROJECT_ROOT" -type d -exec chmod 755 {} \;
find "$PROJECT_ROOT" -type f -exec chmod 644 {} \;

# ---- 3b) SELinux ----
# [ALMA] AlmaLinux trae SELinux en modo "enforcing" por defecto. httpd solo
# puede leer archivos con el contexto httpd_sys_content_t, y necesita
# httpd_sys_rw_content_t si la app escribe en disco (logs, uploads, cache).
if [ "$CONFIG_SELINUX" -eq 1 ] && command -v semanage >/dev/null 2>&1; then
  echo "==> Aplicando contexto SELinux a $PROJECT_ROOT ..."
  semanage fcontext -a -t httpd_sys_content_t "${PROJECT_ROOT}(/.*)?" 2>/dev/null || true
  restorecon -Rv "$PROJECT_ROOT" >/dev/null

  # Si tu app escribe en subcarpetas (storage/, cache/, uploads/), descomentá
  # y ajustá las rutas para darles permiso de escritura bajo SELinux:
  # for DIR in storage cache uploads; do
  #   if [ -d "$PROJECT_ROOT/$DIR" ]; then
  #     semanage fcontext -a -t httpd_sys_rw_content_t "${PROJECT_ROOT}/${DIR}(/.*)?" 2>/dev/null || true
  #     restorecon -Rv "$PROJECT_ROOT/$DIR" >/dev/null
  #   fi
  # done

  # Si la app hace conexiones salientes (DB remota, APIs externas, SMTP):
  setsebool -P httpd_can_network_connect on
  if [ "$DB_HOST" != "localhost" ] && [ "$DB_HOST" != "127.0.0.1" ]; then
    setsebool -P httpd_can_network_connect_db on
  fi
else
  echo "==> (--no-selinux o semanage no disponible) Se omite configuración de SELinux."
fi

# ---- 4) VirtualHost de Apache (httpd) ----
if [ "$CONFIG_VHOST" -eq 1 ]; then
  # [ALMA] httpd no usa sites-available/sites-enabled ni a2ensite/a2enmod.
  # Los .conf sueltos en /etc/httpd/conf.d/ se cargan automáticamente.
  CONF="/etc/httpd/conf.d/${PROJECT}.conf"
  echo "==> Escribiendo $CONF ..."

  if [ "$HTTP_PORT" != "80" ] && [ "$HTTP_PORT" != "443" ]; then
    LISTEN_CONF="/etc/httpd/conf.d/00-listen-${HTTP_PORT}.conf"
    if [ ! -f "$LISTEN_CONF" ]; then
      echo "Listen $HTTP_PORT" > "$LISTEN_CONF"
    fi
    # [ALMA] SELinux por defecto solo permite a httpd escuchar en puertos
    # "conocidos" (80, 443, 8080, etc). Si usás un puerto no estándar:
    if [ "$CONFIG_SELINUX" -eq 1 ] && command -v semanage >/dev/null 2>&1; then
      semanage port -a -t http_port_t -p tcp "$HTTP_PORT" 2>/dev/null || \
      semanage port -m -t http_port_t -p tcp "$HTTP_PORT" 2>/dev/null || true
    fi
  fi

  cat > "$CONF" <<VHOST
<VirtualHost *:$HTTP_PORT>
    ServerName $SERVER_NAME
    DocumentRoot $PROJECT_ROOT/public

    <Directory $PROJECT_ROOT/public>
        Options -Indexes +FollowSymLinks
        AllowOverride All
        Require all granted
    </Directory>

    ErrorLog  /var/log/httpd/${PROJECT}_error.log
    CustomLog /var/log/httpd/${PROJECT}_access.log combined
</VirtualHost>
VHOST

  # mod_rewrite viene habilitado por defecto en httpd de AlmaLinux
  # (se carga desde /etc/httpd/conf.modules.d/00-base.conf), así que no
  # hace falta un equivalente a a2enmod.
  apachectl configtest
  systemctl reload httpd
else
  echo "==> (--no-vhost) Se omite la configuración de Apache."
fi

# ---- 5) Firewall ----
if [ "$CONFIG_FIREWALL" -eq 1 ] && command -v firewall-cmd >/dev/null 2>&1; then
  echo "==> Abriendo puerto $HTTP_PORT en firewalld..."
  if [ "$HTTP_PORT" = "80" ]; then
    firewall-cmd --permanent --add-service=http >/dev/null
  elif [ "$HTTP_PORT" = "443" ]; then
    firewall-cmd --permanent --add-service=https >/dev/null
  else
    firewall-cmd --permanent --add-port="${HTTP_PORT}/tcp" >/dev/null
  fi
  firewall-cmd --reload >/dev/null
else
  echo "==> (--no-firewall o firewalld no disponible) Se omite configuración de firewall."
fi

echo
echo "============================================================"
echo " OK — $APP_NAME instalado."
echo "    URL:       $APP_URL"
echo "    Base:      $DB_NAME  (usuario $DB_USER)"
echo "    DocRoot:   $PROJECT_ROOT/public"
echo "============================================================"
