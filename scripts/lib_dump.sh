...
# Tablas que tiene que traer sí o sí cualquier respaldo de SportTime. Sirve
...
#   0 = parece un respaldo completo de SportTime
...
validar_dump() {
    ...
    if [ "${#faltantes[@]}" -gt 0 ]; then
        echo "ERROR: a $etiqueta le faltan tablas de SportTime: ${faltantes[*]}" >&2
        problemas=1
    fi
