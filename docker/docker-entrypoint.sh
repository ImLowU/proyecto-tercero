...
PROJECT_SSL_DIR="/var/www/sporttime/ssl"
...
sed -i "s/^\(\s*ServerName\).*/\1 ${SERVER_NAME}/" /etc/apache2/sites-available/sporttime-ssl.conf
...
# Sin esto, entrar por un puerto HTTP no estándar (ej. 8082 en desarrollo
...
# usa la imagen de MySQL para correr schema.sql/seed_sporttime.sql en la
# primera inicialización — ese servidor se apaga y el definitivo tarda unos
