...
#    App (HTTP):  http://localhost:8082   (redirige automáticamente a HTTPS, 301)
#    App (HTTPS): https://localhost:8443  (certificado autofirmado hasta que coloques uno real)
#    phpMyAdmin:  http://localhost:8083
```

La base de datos (`sporttime`) se inicializa automáticamente con `database/schema.sql` +
`database/seed_sporttime.sql` la primera vez que arranca el contenedor `db` (volumen vacío).
Si el volumen `sporttime_mysql_data` ya existe, no se vuelve a cargar.
...
> `dependency failed to start: container sporttime_db is unhealthy`, esperá unos segundos,
...
El puerto HTTP (`8082` en local, `80` en producción) **nunca sirve contenido**: solo redirige
...
mkcert sporttime.local
# genera sporttime.local.pem y sporttime.local-key.pem

# 3. Copiarlos al proyecto con los nombres que espera el entrypoint:
cp sporttime.local.pem     ssl/certificate.crt
cp sporttime.local-key.pem ssl/private.key
...
Ver `docs/CREDENCIALES.md` (no versionado).
