...
<section class="section">
    ...
    foreach ($rondasConPartidos as $bloque) {
      ...
      foreach ($partidos as $partido) {
        ...
        $matchesForCalendar[] = [
          ...
          'fecha_fin_real' => $partido['fecha_fin_real'] ?? null,
          'fecha_inicio_real' => $partido['fecha_inicio_real'] ?? null,
  ...
  <?php if ($torneo['estado'] === 'en_curso' || $torneo['estado'] === 'finalizado'): ?>
    ...
    <script>
      ...
      (function () {
        ...
        function parseDateParts(input) {
          ...
          const timePart = raw.includes('T') ? (raw.split('T')[1] || '00:00:00') : (raw.includes(' ') ? (raw.split(' ')[1] || '00:00:00') : '00:00:00');
          const [hour = '00', minute = '00', second = '00'] = (timePart || '00:00:00').split(':').map(String);
        ...
        function isResolvedBeforeScheduled(match) {
          if (!match || !match.fecha) return false;
          if (match.estado === 'finalizado' || match.estado === 'cancelado') return true;

          const scheduled = parseDateParts(match.fecha);
          const realEnd = parseDateParts(match.fecha_fin_real);
          if (!scheduled || !realEnd) return false;

          const scheduledKey = Date.UTC(scheduled.year, scheduled.month - 1, scheduled.day, scheduled.hour, scheduled.minute, scheduled.second);
          const realEndKey = Date.UTC(realEnd.year, realEnd.month - 1, realEnd.day, realEnd.hour, realEnd.minute, realEnd.second);

          return realEndKey < scheduledKey;
        }

        const upcomingMatches = sortMatches(MATCHES.filter((match) => {
          if (!sameOrAfterToday(match.fecha)) return false;
          return !isResolvedBeforeScheduled(match);
        }));
