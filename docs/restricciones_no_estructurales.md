# Restricciones No Estructurales (RNE) — SportTime
...
| 63 | La matriz sembrada reproduce la sección 5 de la letra: el organizador configura torneos asignados y carga resultados, pero no gestiona el padrón de participantes ni de equipos. | `database/seed_sporttime.sql` → bloque PERMISOS; verificado en `tests/permisos_test.php` |
...
| 107 | Un host que solo empieza igual que el propio (`sporttime.local.sitio-falso.com`) no cuenta como propio. | `Url::esHostPropio()` |
