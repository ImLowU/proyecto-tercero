...
1. MySQL crea el volumen de datos desde cero y carga `database/schema.sql` y
   `database/seed_sporttime.sql`. En una máquina con disco lento **pasa de cuatro
...
| phpMyAdmin | http://localhost:8081 — usuario `sporttime_user`, contraseña la de `DB_PASS` |
...
marca en el volumen `sporttime_estado`. Un `restart` o un `up` no los vuelven a
...
(`sporttime_mysql_data`, `sporttime_apache_logs`, `sporttime_estado`), que es
...
```bash
docker volume ls | grep sporttime     # no debería listar ninguno
```
...
**`dependency failed to start: container sporttime_db is unhealthy`**
