# Documentación Técnica — SportTime
...
app:  PHP 8.2 + Apache (puertos 8082 HTTP / 8443 HTTPS en local; 8080/443 en AlmaLinux)
db:   MySQL 8.0        (sin publicar al host — solo accesible dentro de sporttime_net)
pma:  phpMyAdmin       (puerto 8083 en local; 8081 en AlmaLinux)
```

El schema y seed (`seed_sporttime.sql`) se cargan automáticamente vía `docker-entrypoint-initdb.d/`.
