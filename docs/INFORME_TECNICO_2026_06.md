...
> **Importante:** este proyecto usa su propia base: `sporttime`.

XAMPP (despliegue activo) — usar `cmd` para no corromper la codificación:

```bat
cmd /c "mysql --default-character-set=utf8mb4 sporttime < sgdm\database\migrations\2026_06_partidos_fechas.sql"
```

Docker (alternativo): `docker compose exec -T db mysql -u sporttime_user -p sporttime < ...`.
`schema.sql` ya incluye las columnas nuevas para instalaciones desde cero.

**Estado:** las migraciones ya fueron aplicadas en `sporttime`.
